...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    if (video) {
      console.log('Video Hero intentando cargar:', video.currentSrc);
      video.muted = true;
      video.play().then(() => {
        console.log('Video Hero reproduciéndose correctamente');
      }).catch((err) => {
        console.warn('Autoplay bloqueado en Hero, esperando interacción:', err);
        // Fallback for browsers that block autoplay
        const handleInteraction = () => {
          video.play().then(() => console.log('Video Hero iniciado tras interacción'));
      ...
      video.addEventListener('error', () => {
        console.error('Error cargando video en Hero:', video.error);
      });
    }
  }, []);

  return (
    <section className="relative min-h-[calc(100vh-110px)] flex flex-col justify-center px-6 md:px-12 py-20 bg-black overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-[var(--color-bg)]/90 via-[var(--color-bg)]/40 to-transparent z-[2]" />
        <div className="absolute inset-0 bg-gradient-to-t from-[var(--color-bg)] via-transparent to-transparent opacity-60 z-[2]" />
      </div>
      ...
      <div className="relative z-10 max-w-[1800px] mx-auto w-full flex justify-between items-end">
        ...
        <motion.div 
          ...
          className="hidden lg:flex w-56 h-56 items-center justify-center mb-12 mr-12 drop-shadow-2xl"
        >
          <img 
